...
v2ray_conf_add_tls() {
    cd /etc/v2ray || exit
    wget --no-check-certificate https://raw.githubusercontent.com/xkdzt/v2_ws_tls_bash/${github_branch}/ssl/config.json -O config.json
    modify_path
    modify_alterid
    modify_inbound_port
    modify_UUID
...
#}
acme_cron_update() {
    wget -N -P /usr/bin --no-check-certificate "https://raw.githubusercontent.com/xkdzt/v2_ws_tls_bash/master/ssl_update.sh"
    if [[ "${ID}" == "centos" ]]; then
